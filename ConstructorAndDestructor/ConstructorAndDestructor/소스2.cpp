...
int main() {
	...
	while (true) {
		...
		if (input == 1) {
			...
			for (int i = 0; i < count; i++) {
				if (number == employee[i]->no) { //사용자가 방금 입력한 사원번호와 배열안에 사원번호가 일치한다면
					delete employee[i]; //사원삭제
					deleteIndex = i; //일치하는 사원이 있다면 해당 인덱스 부여
					break;
				}
			}
			if (deleteIndex >= 0) { //삭제가 됐나? 알아보는것
				for (int i = deleteIndex; i < count - 1; i++) { //현재 적재된 인덱스부터 배열 끝까지
...
char* getString(const char* prompt) { //사용자에게 문자열을 입력받는다
	...
	char* input = new char[100]; //힙에 생기므로 문자열이 사라지지 않는다.
	printf("%s", prompt);
	fseek(stdin, 0, SEEK_END);
